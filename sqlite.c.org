 ...
 *
 */
int ChangePasswd( int uid , char *vncpasswd )
{

	memset( username_global, 0, sizeof( username_global ) );
	...
	if( SQLITE_OK != ret ){
		...
		strcpy( vncpasswd, username_global );
	}
...
int print_resp( void *get_prm, int col_cnt, char ** row_txt, char **col_name )
{
	...
	for( i = 0; i < VNCPW_LEN; i++ ){
		...
		if( 0 <= rand_num && rand_num < 4 ){			// ( # | $ | % | & )
			...
			printf("(sqlite.c):print_resp(): Failed to generate VNCpassword.");
		}
	}
	// 終端文字処理
	onetime_passwd[i] = '\0';
